...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "ha:p:u:f:r:c:v:w:t:d", long_options, &option_index)) != -1) {
        switch(c){
    ...
    if (sock < 0) {
        perror(BOLD RED "?? socket error ??" RESET);
        return 1;
    }
    ...
    if (inet_pton(AF_INET, cfg.address, &remote.sin_addr) != 1) {
        fprintf(stderr, BOLD RED "?? invalid address: %s ??\n" RESET, cfg.address);
        close(sock);
        return 1;
    }

    if (connect(sock, (struct sockaddr*)&remote, sizeof(remote)) < 0) {
        perror(BOLD RED "?? connect error ??" RESET);
        close(sock);
        return 1;
    }
    ...
    if (cfg.rawdata != NULL) {
        len = parse_hex(cfg.rawdata, rawbuf);
    ...
    while (total < len) {
        ...
        if (n <= 0) {
            perror(BOLD RED "?? write error ??" RESET);
            close(sock);
            return 1;
        }
        total += n;
